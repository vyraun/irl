...
def gen_expert_data(save_dir, expert=True, camera_angle=0, batch_size=6000, burn_in=10, save_steps=10):
    ...
    env = TfEnv(normalize(GymEnv('Impoint-v0')))
    ...
    total_iters = int(math.ceil((1.0*burn_in + save_steps) / batch_size) + 1)
...
if __name__ == '__main__':
    ...
    save_steps = 10000
    batch_size = 6000
    burn_in_expert = 12*batch_size
    burn_in_bad = batch_size

    #gen_expert_data(expert_one_path, expert=True, camera_angle=1, burn_in=burn_in_expert,
    #                save_steps=save_steps, batch_size=batch_size)
    #gen_expert_data(expert_two_path, expert=True, camera_angle=0,
